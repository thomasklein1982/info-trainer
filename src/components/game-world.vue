<template>
  <AppPreview :width="width" height="auto" style="position: relative; z-index: 1;">
    <template v-if="gameworld">
      <JFrame :layout="gameworld.width+''" class="world" style="background-color: lightgreen">
        <template v-for="(f,i) in gameworld.fields">
          ...
          <template v-else>
            <JLabel style="outline: 1pt dotted darkgreen; aspect-ratio: 1;" :style="{'grid-row': f.y, 'grid-column': f.x}"/>
          </template>
        </template>
